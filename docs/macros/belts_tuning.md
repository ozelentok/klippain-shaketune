...
The `SHAKETUNE_TEST_BELTS` macro is dedicated for CoreXY machines where it can help you to diagnose belt path problems by measuring and plotting the differences between their behavior. It will also help you tension your belts at the same tension.
...
Then, call the `SHAKETUNE_TEST_BELTS` macro and look for the graphs in the results folder. Here are the parameters available:
...
|KEEP_CSV|1|Weither or not to keep the CSV data files alonside the PNG graphs|
