...
from .common import compute_spectrogram, detect_peaks, load_accelerometer_data
...
SignalData = namedtuple(
    "CalibrationData",
    ["freqs", "psd", "peaks", "paired_peaks", "unpaired_peaks"])
...
def compute_curve_similarity_factor(signal1, signal2):
    ...
    similarity = peak_value / (np.sqrt(
        np.sum(psd1**2) * np.sum(psd2_interp**2)))

    # Apply sigmoid scaling to get better numbers and get a final percentage value
    scaled_similarity = sigmoid_scale(-np.log(1 - similarity),
                                      CURVE_SIMILARITY_SIGMOID_K)
...
def pair_peaks(peaks1, freqs1, psd1, peaks2, freqs2, psd2):
    ...
    while unpaired_peaks1 and unpaired_peaks2:
        ...
        p1, p2 = pair
        paired_peaks.append(
            ((p1, freqs1[p1], psd1[p1]), (p2, freqs2[p2], psd2[p2])))
...
def interpolate_2d(target_x: np.ndarray, target_y: np.ndarray,
    ...
    interpolated_data = griddata(source_points,
                                 source_values,
                                 target_points,
                                 method="nearest")
    interpolated_data = interpolated_data.reshape(
        (len(target_y), len(target_x)))
...
def compute_combined_spectrogram(
...
def compute_mhi(combined_data, similarity_coefficient,
                num_unpaired_peaks) -> tuple[int, str]:
    ...
    base_percentage = (np.log1p(total_variability_metric) /
                       np.log1p(DC_THRESHOLD_METRIC)) * 100

    # Adjust the percentage based on the similarity_coefficient to add a grain of salt
    adjusted_percentage = base_percentage * (1 - DC_GRAIN_OF_SALT_FACTOR *
    ...
    final_percentage = (
        1 - peak_confidence) * adjusted_percentage + peak_confidence * 100
...
def mhi_lut(mhi: int) -> str:
    ranges = [(0, 30, "Excellent mechanical health"),
...
def plot_compare_frequency(ax, csv_paths: list[Path], signal1, signal2,
                           similarity_factor, max_freq):
    # Get the belt name for the legend to avoid putting the full file name
    signal1_belt = (csv_paths[0].stem).split("_")[-1][0]
    signal2_belt = (csv_paths[1].stem).split("_")[-1][0]
    ...
    ax.plot(signal1.freqs,
            signal1.psd,
            label="Belt " + signal1_belt,
            color=KLIPPAIN_COLORS["purple"],
            )
    ax.plot(signal2.freqs,
            signal2.psd,
            label="Belt " + signal2_belt,
    ...
    ax.axhline(y=peaks_warning_threshold,
               color="black",
               linestyle="--",
               linewidth=0.5)
    ax.fill_between(signal1.freqs,
                    0,
                    peaks_warning_threshold,
                    color="green",
                    alpha=0.15,
                    label="Relax Region",
                    )
    ...
    for _, (peak1, peak2) in enumerate(signal1.paired_peaks):
        ...
        amplitude_offset = abs(
            ((signal2.psd[peak2[0]] - signal1.psd[peak1[0]]) /
             max(signal1.psd[peak1[0]], signal2.psd[peak2[0]])) * 100)
        frequency_offset = abs(signal2.freqs[peak2[0]] -
                               signal1.freqs[peak1[0]])
        offsets_table_data.append([
            f"Peaks {label}", f"{frequency_offset:.1f} Hz",
            f"{amplitude_offset:.1f} %",
        ])

        ax.plot(signal1.freqs[peak1[0]],
                signal1.psd[peak1[0]],
                "x",
                color="black")
        ax.plot(signal2.freqs[peak2[0]],
                signal2.psd[peak2[0]],
                "x",
                color="black")
        ax.plot([signal1.freqs[peak1[0]], signal2.freqs[peak2[0]]],
    ...
    for peak in signal1.unpaired_peaks:
        ...
        ax.annotate(str(unpaired_peak_count + 1),
    ...
    ax2.plot([], [],
             " ",
             label=f"Estimated similarity: {similarity_factor:.1f}%")
    ax2.plot([], [],
             " ",
             label=f"Number of unpaired peaks: {unpaired_peak_count}")
    ...
    ax.set_title(
        "Belts Frequency Profiles (estimated similarity: {:.1f}%)".format(
            similarity_factor),
        fontsize=14,
    ...
    if len(offsets_table_data) > 0:
        ...
        offset_table = ax.table(cellText=offsets_table_data,
                                colLabels=columns,
...
def plot_difference_spectrogram(ax, signal1, signal2, t, bins,
                                combined_divergent, textual_mhi, max_freq):
    ax.set_title(f"Differential Spectrogram",
                 fontsize=14,
                 color=KLIPPAIN_COLORS["dark_orange"],
                 weight="bold",
                 )
    ...
    colors = [
        KLIPPAIN_COLORS["dark_orange"], KLIPPAIN_COLORS["orange"], "white",
        KLIPPAIN_COLORS["purple"], KLIPPAIN_COLORS["dark_purple"],
    ]
    cm = matplotlib.colors.LinearSegmentedColormap.from_list(
        "klippain_divergent", list(zip([0, 0.25, 0.5, 0.75, 1], colors)))
    norm = matplotlib.colors.TwoSlopeNorm(vmin=np.min(combined_divergent),
                                          vcenter=0,
                                          vmax=np.max(combined_divergent))
    ax.imshow(combined_divergent.T,
              cmap=cm,
              norm=norm,
              aspect="auto",
    ...
    for _, peak in enumerate(signal1.unpaired_peaks):
        ax.axvline(signal1.freqs[peak],
                   color=KLIPPAIN_COLORS["red_pink"],
                   linestyle="dotted",
                   linewidth=1.5,
                   )
        ax.annotate(f"Peak {unpaired_peak_count + 1}",
...
def compute_signal_data(data, max_freq):
    ...
    psd = calibration_data.get_psd("all")[calibration_data.freq_bins <=
                                          max_freq]

    _, peaks, _ = detect_peaks(psd, freqs,
                               PEAKS_DETECTION_THRESHOLD * psd.max())

    return SignalData(freqs=freqs,
                      psd=psd,
                      peaks=peaks,
                      paired_peaks=None,
                      unpaired_peaks=None)
...
def graph_belts_test_results(csv_paths: list[Path],
                             max_freq=200.0) -> matplotlib.figure.Figure:
    datas = [load_accelerometer_data(p) for p in csv_paths]
    if not datas or len(datas) > 2:
        raise ValueError(
            "Incorrect number of .csv files used (this function needs exactly two files to compare them)!"
        )
    ...
    combined_sum, combined_divergent, bins, t = compute_combined_spectrogram(
        datas[0], datas[1])
    del datas

    # Pair the peaks across the two datasets
    paired_peaks, unpaired_peaks1, unpaired_peaks2 = pair_peaks(
        signal1.peaks, signal1.freqs, signal1.psd, signal2.peaks, signal2.freqs,
        signal2.psd,
    )
    signal1 = signal1._replace(paired_peaks=paired_peaks,
                               unpaired_peaks=unpaired_peaks1)
    signal2 = signal2._replace(paired_peaks=paired_peaks,
                               unpaired_peaks=unpaired_peaks2)
    ...
    mhi, textual_mhi = compute_mhi(
        combined_sum, similarity_factor,
        len(signal1.unpaired_peaks) + len(signal2.unpaired_peaks),
    )
    print(
        f"[experimental] Mechanical Health Indicator: {textual_mhi.lower()} ({mhi:.1f}%)"
    )
    ...
    try:
        ...
        dt = datetime.strptime(
            f"{filename.split('_')[1]} {filename.split('_')[2]}",
            "%Y%m%d %H%M%S")
        title_line2 = dt.strftime("%x %X")
    except:
        print(
            f"Warning: CSV filenames look to be different than expected ({csv_paths[0]}, {csv_paths[1]})"
        )
        title_line2 = csv_paths[0].name + " / " + csv_paths[1].name
    ...
    # Plot the graphs
    plot_compare_frequency(ax1, csv_paths, signal1, signal2, similarity_factor,
                           max_freq)
    plot_difference_spectrogram(ax2, signal1, signal2, t, bins,
                                combined_divergent, textual_mhi, max_freq)
